...
float vol_Cilindro(float raio, float altura);
float vol_Calota(float raio, float altura);
...
float vol_Cilindro(float raio, float altura_cil)
{
    float volume = Pi * pow(raio,2) * altura_cil;

    return volume;
}

float vol_Calota(float raio, float altura_calota)
{
    float volume = (Pi * pow(altura_calota,2) * (3*raio - altura_calota))/3;  //Não funcionou ??
